...
tongue_statistical_images = image.load_img(list(sub_file_path.rglob('./tongue_vs_rest/spm*_0001.nii'))[0].as_posix())
...
dataset = OrderedDict(
    ...
    tongue_vs_rest=dataset["tongue_vs_rest"],
    tongue_vs_dispersion=dataset["tongue_vs_dispersion"],
    tongue_vs_time=dataset["tongue_vs_time"],
    tongue_dispersion=dataset["tongue_dispersion"],
    tongue_time=dataset["tongue_time"],
)
...
# create a figure with multiple axes to plot each anatomical image
fig, axes = plt.subplots(nrows=num_maps, ncols=1)
fig.set_size_inches((20, 2 * num_files))
faxes = axes.flatten()
...
#     y=.5,
# )
display.savefig(target_path / "misc" / "level_2_results_allinone_axial_tongue.png")
